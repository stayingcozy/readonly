...
public:
  ...
  // run agent for auth -> clean poweroff+wait -> (atomically) keep overlay +
  // write .meta any failure leaves registry untouched
  ...
  Result<int> run(std::string_view name, const fs::path &target,
                  const MaskSpec &);
