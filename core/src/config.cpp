...
namespace {
    Result<fs::path> home_dir() {
        ...
        return fail("cannot determine home directory (HOME unset and no passwd entry)");
    }
...
Result<Paths> Paths::at(fs::path root, fs::path scratch) {
    ...
    if (auto r = ensure_dir(p.root_);            !r) return std::unexpected(r.error());
    if (auto r = ensure_dir(p.root_ / "agents"); !r) return std::unexpected(r.error());
    if (auto r = ensure_dir(p.scratch_);         !r) return std::unexpected(r.error());
    return p;
}
...
Result<GlobalConfig> GlobalConfig::load(const Paths& paths) {
    ...
    if (!fs::exists(file)) return GlobalConfig{}; // first run: silent defaults
    ...
    for (const auto& [key, value] : *entries) {
        if (key == "default_mask")        cfg.default_mask = kv::split_list(value);
        else if (key == "accel_override") cfg.accel_override = value;
