...
std::string now_iso8601() {
  const auto t = std::chrono::floor<std::chrono::seconds>(
...
Result<void> Registry::validate_name(std::string_view name) {
  if (name.empty())
    return fail("agent name is empty");
  if (name.size() > 64)
    return fail("agent name is longer than 64 characters");

  const auto ok_first = [](char c) {
    return (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9');
  };
  const auto ok_rest = [&](char c) {
    return ok_first(c) || c == '.' || c == '_' || c == '-';
  };

  if (!ok_first(name.front()))
    return fail(std::format(
        "agent name '{}' must start with a lowercase letter or digit", name));

  for (const char c : name) {
    if (!ok_rest(c)) {
      return fail(std::format("agent name '{}' contains invalid character '{}' "
                              "(allowed: a-z 0-9 . _ -)",
                              name, c));
    }
  }
...
Result<AgentDescriptor> Registry::resolve(std::string_view name) const {
  if (auto v = validate_name(name); !v)
    ...
    if (auto text = kv::read_file(mp)) {
      if (auto entries = kv::parse_kv(*text, mp)) {
        for (const auto &[key, value] : *entries) {
          if (key == "surface")
            d.surface =
                (value == "gui") ? shared::Surface::Gui : shared::Surface::Cli;
          else if (key == "run" && !value.empty())
            d.run_cmd = value;
          else if (key == "install_cmd")
            d.install_cmd = value;
          else if (key == "created")
            d.created = value;
        }
...
Result<void> Registry::write(const AgentDescriptor &d) const {
  if (auto v = validate_name(d.name); !v)
    return std::unexpected(v.error());
  ...
  const std::string body = std::format(
      ...
      "install_cmd={}\n"
      "created={}\n",
      d.name, d.surface == shared::Surface::Gui ? "gui" : "cli", d.run_cmd,
      d.install_cmd, d.created.empty() ? now_iso8601() : d.created);
...
Result<std::vector<AgentDescriptor>> Registry::list() const {
  ...
  std::error_code ec;
  if (!fs::is_directory(paths_.agents_dir(), ec))
    return out; // nothing installed yet

  for (fs::directory_iterator it(paths_.agents_dir(), ec), end;
       !ec && it != end; it.increment(ec)) {
    if (!it->is_regular_file(ec) || it->path().extension() != ".qcow2")
      continue;

    const std::string name = it->path().stem().string();
    if (!validate_name(name))
      continue; // junk in the trunk but must not break me!

    if (auto d = resolve(name))
      out.push_back(std::move(*d));
  }

  std::ranges::sort(out, {},
                    &AgentDescriptor::name); // stable output across filesystems
  return out;
}
