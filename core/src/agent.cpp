...
namespace {
// RAII: remove overlay unless explicitly kept; run overlay discarded on every path
class OverlayGuard {
public: 
    explicit OverlayGuard(fs::path p) : path_(std::move(p)) {}
    ~OverlayGuard() { if (armed_) { std::error_code ec; fs::remove(path_, ec); } }
...
Result<int> interactive_session(Vm& vm, std::string_view command) {
    ...
    if (auto r = vs->send_run(command); !r) return std::unexpected(r.error());
...
Result<AgentManager::Deps> AgentManager::resolve_deps() const {
    ...
    if (!fs::is_regular_file(d.kernel, ec))
        return fail(std::format("kernel missing: {} (run `readonly setup`)", d.kernel.string()));
    return d;
}
...
Result<int> AgentManager::run(std::string_view name, const fs::path& target,
    ...
    if (auto r = Snapshot::create_overlay(registry_.overlay_path(name), run_overlay); !r)
        return std::unexpected(r.error());
    OverlayGuard overlay_guard(run_overlay); // discarded on every exit path

    // 3. boot
    QemuConfig cfg = make_config(run_overlay, deps->kernel, scratch->src(), scratch->out());
...
Result<void> AgentManager::install(std::string_view install_cmd,
    ...
    if (auto v = Registry::validate_name(name); !v) return std::unexpected(v.error());
    if (registry_.exists(name))
        return fail(std::format("agent '{}' already installed (uninstall it first)", name));
    ...
    QemuConfig cfg = make_config(staging, deps->kernel, scratch->src(), scratch->out());
    auto vm = Vm::launch(cfg);
    if (!vm) return std::unexpected(vm.error());

    auto vs = VsockClient::connect(3, shared::kVsockPort);
    if (!vs) { vm->kill(); return std::unexpected(vs.error()); }

    // Session 1: run the install cmd interactively
    {
        if (auto r = vs->send_run(install_cmd); !r) { vm->kill(); return std::unexpected(r.error()); }
        auto term = TerminalSession::enter();
        if (!term) { vm->kill(); return std::unexpected(term.error()); }
        auto code = term->pump(*vs);
        if (!code) { vm->kill(); return std::unexpected(code.error()); }
        if (*code != 0) { vm->kill(); return fail(std::format("install command exit {}", *code)); }        
    }
    ...
    // reconnect: supervisor looped back to accept() after sess 1
    {
        ...
        if (!vs2) { vm->kill(); return std::unexpected(vs2.error()); }
        if (auto r = vs2->send_run(name); !r) { vm->kill(); return std::unexpected(r.error()); }
        auto term = TerminalSession::enter();
        if (!term) { vm->kill(); return std::unexpected(term.error()); }
        (void)term->pump(*vs2);    // exit code irrelevant; user need auth then dip
    }

    // Session 3: clean flush + poweroff, WAIT for qemu to exit before keeping overlay
    {
        auto vs3 = VsockClient::connect(3, shared::kVsockPort);
        if (!vs3) { vm->kill(); return std::unexpected(vs3.error()); }
        (void)vs3->send_run("sync; poweroff");     // fire and forget; guest tears down socket
    }
    auto exit_code = vm->wait();  // blocks until guest has flushed and powered off
    if (!exit_code) { vm->kill(); return std::unexpected(exit_code.error()); }
    ...
    if (ec) return fail(std::format("cannot commit overlay to {}: {}", final_path.string(), ec.message()));
