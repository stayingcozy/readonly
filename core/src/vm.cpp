...
AccelInfo detect_accel() {
    ...
    if (fd >= 0) { ::close(fd); return {Accel::Kvm, false}; }
    if (errno == EACCES || errno == EPERM) return {Accel::None, true};  // in-group fix needed
...
namespace {
struct AccelFlags { std::string machine_accel; std::string cpu; };

AccelFlags accel_flags(Accel a) {
    switch (a) {
        case Accel::Kvm:  return {"kvm", "host"};
        case Accel::Hvf:  return {"hvf", "host"};
        case Accel::Whpx: return {"whpx", "max"};
        case Accel::None: return {"tcg", "max"}; 
    }
    return {"tcg", "max"};
}
} // namespace
...
std::vector<std::string> Vm::build_argv(const QemuConfig& c) {
    ...
    std::vector<std::string> a{
        "qemu-system-x86_64",
        "-machine", std::format("q35,accel={}", maccel),
        "-cpu", cpu,
        "-m", std::to_string(c.memory_mb),
        "-kernel", c.kernel.string(),
        "-append", c.append,   // one argv element — spaces need no quoting without a shell
        "-drive", std::format("file={},if=virtio,format=qcow2", c.image.string()),
        "-netdev", "user,id=n0",
        "-device", "virtio-net-pci,netdev=n0",
        "-fsdev", std::format("local,id=srcfs,path={},security_model=none,readonly=on",
                              c.src_share.string()),
        "-device", "virtio-9p-pci,fsdev=srcfs,mount_tag=src",
        "-fsdev", std::format("local,id=outfs,path={},security_model=none",
                              c.out_share.string()),
        "-device", "virtio-9p-pci,fsdev=outfs,mount_tag=out",
        "-device", std::format("vhost-vsock-pci,guest-cid={}", c.guest_cid),
    };

    if (c.serial_log) {
        a.insert(a.end(), {"-display", "none",
                            "-serial", std::format("file:{}", c.serial_log->string())});
...
Result<Vm> Vm::launch(const QemuConfig& c) {
    if (auto r = must_be_file(c.image,  "overlay image"); !r) return std::unexpected(r.error());
    if (auto r = must_be_file(c.kernel, "kernel bzImage"); !r) return std::unexpected(r.error());
    if (auto r = must_be_dir(c.src_share, "src share");   !r) return std::unexpected(r.error());
    if (auto r = must_be_dir(c.out_share, "out share");   !r) return std::unexpected(r.error());
    ...
    if (w == pid) {
        ...
        return fail(std::format(
            "qemu exited immediately (code {}) — check the argv (readonly __boot prints it)", code));
    }
...
Result<int> Vm::wait() {
    if (pid_ == -1) return fail("no VM process to wait on (already terminated)");
    int status = 0;
...
Vm& Vm::operator=(Vm&& o) noexcept {
    if (this != &o) { kill(); pid_ = o.pid_; o.pid_ = -1; }
    return *this;
}
