...
bool MaskSpec::matches(std::string_view basename) const {
  for (const auto &n : names) {
    ...
    if (basename.size() > n.size() && basename.starts_with(n) &&
        basename[n.size()] == '.')
      return true;
  }
  return false;
...
Result<RunScratch> RunScratch::create(const Paths &paths) {
  ...
  for (int attempt = 0; attempt < 4 && s.run_dir_.empty(); ++attempt) {
    ...
    if (ec)
      return fail(std::format("cannot create scratch dir {}: {}", dir.string(),
                              ec.message()));
  ...
  if (ec)
    return fail(
        std::format("cannot create {}: {}", s.src_.string(), ec.message()));
  fs::create_directory(s.out_, ec);
  if (ec)
    return fail(
        std::format("cannot create {}: {}", s.out_.string(), ec.message()));
...
RunScratch::RunScratch(RunScratch &&o) noexcept
    : run_dir_(std::move(o.run_dir_)), src_(std::move(o.src_)),
...
Result<MirrorStats> RunScratch::mirror_source(const fs::path &target,
                                              const MaskSpec &mask) {
  std::error_code ec;
  ...
  if (ec)
    return fail(std::format("cannot resolve source path {}: {}",
                            target.string(), ec.message()));
  if (!fs::is_directory(root, ec))
    return fail(
        std::format("source path {} is not a directory", root.string()));
  ...
  for (; it != end; it.increment(ec)) {
    if (ec)
      return fail(
          std::format("error walking {}: {}", root.string(), ec.message()));
    ...
    if (mask.matches(name)) {
      ...
      if (fs::is_directory(st))
        it.disable_recursion_pending(); // skip whole subtree
      continue;
    }
    ...
    if (fs::is_directory(st)) {
      ...
      if (dec)
        return fail(
            std::format("cannot create {}: {}", dst.string(), dec.message()));
      ++stats.dirs;
      ...
      if (!lec) {
        ...
        if (cec)
          return fail(std::format("cannot mirror {} -> {}: {}", p.string(),
