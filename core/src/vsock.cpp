...
int recv_all(int fd, void* buf, std::size_t n) {
    auto* p = static_cast<std::uint8_t*>(buf); std::size_t got = 0;
    while (got < n) {
...
int send_all(int fd, const void* buf, std::size_t n) {
    const auto* p = static_cast<const std::uint8_t*>(buf); std::size_t put = 0;
    while (put < n) {
...
Result<void> send_frame(int fd, FrameType type, const void* payload, std::uint32_t len) {
    std::uint8_t hdr[5];
    hdr[0] = static_cast<std::uint8_t>(type);
    hdr[1] = std::uint8_t(len);       hdr[2] = std::uint8_t(len >> 8);
    hdr[3] = std::uint8_t(len >> 16); hdr[4] = std::uint8_t(len >> 24);
    if (send_all(fd, hdr, 5) != 1) return fail("vsock write failed (header)");
    if (len && send_all(fd, payload, len) != 1) return fail("vsock write failed (payload)");
    return {};
}
} // namespace
...
Result<VsockClient> VsockClient::connect(unsigned cid, unsigned port) {
    ...
    for (int i = 0; i < kAttempts; ++i) {
        ...
        if (fd < 0) return fail(std::format("AF_VSOCK socket failed: {}", std::strerror(errno)));
        ...
        if (::connect(fd, reinterpret_cast<sockaddr*>(&addr), sizeof addr) == 0) {
            VsockClient c; c.fd_ = fd; return c;
        }
        ::close(fd);
    ...
    return fail(std::format(
            ...
            "(supervisor not started? check the serial log)", cid, port));
}

Result<void> VsockClient::send_run(std::string_view cmd) {
    return send_frame(fd_, FrameType::Run, cmd.data(), static_cast<std::uint32_t>(cmd.size()));
}
...
Result<VsockClient::Frame> VsockClient::next_frame() {
    ...
    if (r == 0) return fail("supervisor closed the connection");
    if (r < 0) return fail(std::format("vsock read failed: {}", std::strerror(errno)));

    Frame f;
    f.type = static_cast<FrameType>(hdr[0]);
    const std::uint32_t len = std::uint32_t(hdr[1]) | (std::uint32_t(hdr[2]) << 8)
    ...
    if (f.type == FrameType::Exit) {
        if (len >= 4) {
            f.exit_code = int(std::uint8_t(payload[0])) | (int(std::uint8_t(payload[1])) << 8) 
                            | (int(std::uint8_t(payload[2])) << 16) | (int(std::uint8_t(payload[3])) << 24);
        }
    } else {
...
VsockClient::~VsockClient() { if (fd_ != -1) ::close(fd_); }
