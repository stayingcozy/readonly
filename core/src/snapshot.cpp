...
Result<void> Snapshot::qemu_img_available() {
    ...
    if (r->exit_code != 0) 
        return fail("qemu-img not found on PATH - install QEMU tools (e.g. qemu-utils)");
    return {};
}

Result<void> Snapshot::create_overlay(const fs::path& backing, const fs::path& overlay) {
    std::error_code ec;

    // backing must exist and resolve to abs. path
    const fs::path backing_abs = fs::canonical(backing, ec);
    if (ec) 
        return fail(std::format("backing image {} not found: {}", backing.string(), ec.message()));
    ...
    auto r = proc::run({"qemu-img", "create",
                        "-f", "qcow2",
                        "-b", backing_abs.string(),
                        "-F", "qcow2",
                        overlay.string()});
    if (!r) return std::unexpected(r.error());
    if (r->exit_code != 0)
        return fail(std::format("qemu-img create failed: {}",
...
Result<void> Snapshot::discard(const fs::path& overlay) {
    ...
    if (ec) 
        return  fail(std::format("cannot remove {}: {}", overlay.string(), ec.message()));
    return {};
}
