...
Result<std::string> read_file(const fs::path &p) {
  ...
  if (!in)
    return fail(std::format("cannot read {}", p.string()));
  std::ostringstream ss;
  ss << in.rdbuf();
  if (in.bad())
    return fail(std::format("error reading {}", p.string()));
  return ss.str();
}
...
Result<void> write_file_atomic(const fs::path &p, std::string_view contents) {
  ...
  if (fd < 0)
    return fail(
        std::format("cannot open {}: {}", tmp.string(), std::strerror(errno)));
...
Result<std::vector<Entry>> parse_kv(std::string_view text,
  ...
  for (std::size_t pos = 0; pos <= text.size();) {
    ...
    if (eq == std::string_view::npos)
      return fail(
          std::format("{}:{}: expected key=value", origin.string(), line_no));
...
std::vector<std::string> split_list(std::string_view s, char delim) {
  ...
  for (std::size_t pos = 0; pos <= s.size();) {
    ...
    const auto part =
        s.substr(pos, (d == std::string_view::npos ? s.size() : d) - pos);
    pos = (d == std::string_view::npos) ? s.size() + 1 : d + 1;

    if (const auto t = trim(part); !t.empty())
      out.emplace_back(t);
  }
  return out;
}
