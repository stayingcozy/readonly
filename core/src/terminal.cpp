...
Result<TerminalSession> TerminalSession::enter() {
  if (!::isatty(STDIN_FILENO))
    return core::fail(
        "stdin is not a terminal (readonly needs an interactive tty)");

  if (::tcgetattr(STDIN_FILENO, &g_orig) != 0)
    return core::fail(
        std::format("tcgetattr failed: {}", std::strerror(errno)));
  ...
  if (::tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) != 0)
    return core::fail(
        std::format("tcsetattr failed: {}", std::strerror(errno)));
...
TerminalSession::TerminalSession(TerminalSession &&o) noexcept
    : active_(o.active_) {
  o.active_ = false;
}
...
void TerminalSession::draw_bar() {
  ...
  for (int c = static_cast<int>(label.size()); c < p.cols; ++c)
    bar += ' ';
  bar += kReset;
  bar += kRestCur;
