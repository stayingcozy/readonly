...
MaskSpec parse_mask(const std::string& csv) {
    ...
    for (char c : csv + ",") {
        if (c == ',') {
            if (!cur.empty()) m.names.push_back(cur);
            cur.clear();
        } else {
            cur += c;
        }
    }
    if (m.names.empty()) m.names.push_back(".env");
    return m;
}
...
int run_agent_form(int argc, char** argv) {
    ...
    app.add_option("--mask", mask, "comma-separated names to exclude from the mirror");
...
void add_debug_commands(CLI::App& app) {
    ...
    app.add_subcommand("__config", "debug: resolved paths + config")
        ...
        ->callback([] {
            auto paths = Paths::discover();
            if (!paths) { std::println(stderr, "error: {}", paths.error().message); return; }
            ...
            if (!cfg) { std::println(stderr, "config error: {}", cfg.error().message); return; }
            std::string masks;
            ...
            std::println("accel:   {}", cfg->accel_override.empty() ? "(auto)" : cfg->accel_override);
        });
    ...
    app.add_subcommand("__accel", "debug: accel detection")
        ...
        ->callback([] {
            ...
            if (info.kvm_present_but_denied)
                std::println("note: /dev/kvm exists but is not accessible — "
    ...
    // __mirror — masked hardlink mirror, left on disk to inspect
    {
        ...
        auto* c = app.add_subcommand("__mirror", "debug: masked hardlink mirror")->group("");
        ...
        c->callback([] {
            ...
            if (!scratch) { std::println(stderr, "error: {}", scratch.error().message); return; }
            auto stats = scratch->mirror_source(path, parse_mask(mask));
            if (!stats) { std::println(stderr, "error: {}", stats.error().message); return; }
            std::println("mirror: {}", scratch->src().string());
            std::println("linked={} copied={} masked={} symlinks_skipped={} special_skipped={} dirs={}",
                         stats->linked, stats->copied, stats->masked,
                         stats->symlinks_skipped, stats->special_skipped, stats->dirs);
    ...
    // __overlay — create a qcow2 overlay on a backing image
    {
        ...
        auto* c = app.add_subcommand("__overlay", "debug: create qcow2 overlay")->group("");
    ...
    // __boot — boot an overlay (serial on stdio; Ctrl-A X to quit qemu)
    {
        ...
        c->callback([] {
            ...
            std::string cmd;
            for (const auto& a : Vm::build_argv(cfg)) { cmd += a; cmd += ' '; }
            std::println("launching: {}\n", cmd);

            auto vm = Vm::launch(cfg);
            if (!vm) { std::println(stderr, "error: {}", vm.error().message); return; }
            std::println("[booted pid {} — Ctrl-A X to quit qemu]", vm->pid());
            auto code = vm->wait();
            if (!code) std::println(stderr, "error: {}", code.error().message);
            else       std::println("\n[qemu exited: {}]", *code);
        });
    }
    ...
    // __exec — boot + connect + run a command (cooked pump, no raw mode)
    {
        ...
        auto* c = app.add_subcommand("__exec", "debug: run a command in the guest")->group("");
        c->add_option("--image", image)->required();
        c->add_option("--kernel", kernel)->required();
        c->add_option("--src", src)->required();
        c->add_option("--out", out)->required();
        ...
        c->callback([] {
            ...
            auto vm = Vm::launch(cfg);
            if (!vm) { std::println(stderr, "boot error: {}", vm.error().message); return; }
            std::println(stderr, "[booting; serial -> readonly-serial.log]");

            auto vs = VsockClient::connect(cfg.guest_cid, shared::kVsockPort);
            if (!vs) { std::println(stderr, "connect error: {}", vs.error().message); vm->kill(); return; }

            std::string joined;
            for (std::size_t i = 0; i < cmd.size(); ++i) { if (i) joined += ' '; joined += cmd[i]; }
            if (auto r = vs->send_run(joined); !r) {
                std::println(stderr, "{}", r.error().message); vm->kill(); return;
            }
            ...
            while (!done) {
                ...
                if (fds[0].revents & POLLIN) {
                    ...
                    if (n > 0) (void)vs->send_stdin({b.data(), static_cast<std::size_t>(n)});
                }
                if (fds[1].revents & (POLLIN | POLLHUP)) {
                    auto f = vs->next_frame();
                    if (!f) { std::println(stderr, "\n[stream ended: {}]", f.error().message); break; }
                    if (f->type == shared::FrameType::Data)
                        ::write(STDOUT_FILENO, f->data.data(), f->data.size());
                    else if (f->type == shared::FrameType::Exit) { exit_code = f->exit_code; done = true; }
                }
            }
            vm->kill();
    ...
    // __run — interactive session with raw mode + gold bar (the real experience)
    {
        ...
        auto* c = app.add_subcommand("__run", "debug: interactive session in the guest")->group("");
        c->add_option("--image", image)->required();
        c->add_option("--kernel", kernel)->required();
        c->add_option("--src", src)->required();
        c->add_option("--out", out)->required();
        ...
        c->callback([] {
            ...
            if (!term) { std::println(stderr, "term error: {}", term.error().message); vm->kill(); return; }

            readonly::core::Result<int> code = readonly::core::fail("session did not run");
            {
            ...
            vm->kill();
            if (!code) std::println(stderr, "session error: {}", code.error().message);
            else       std::println("[exited: {}]", *code);
        });
    }
}
...
int main(int argc, char** argv) {
    ...
    // setup — download base image + kernel from S3 into ~/.readonly (not yet implemented)
    app.add_subcommand("setup", "download the base image and prepare ~/.readonly")
        ->callback([] {
            std::println(stderr, "setup: not yet implemented — "
    ...
    {
        auto* c = app.add_subcommand("install", "install + authenticate an agent, then snapshot");
        c->add_option("cmd", install_cmd, "vendor install command (single-quoted)")->required();
        ...
        c->callback([&] {
            ...
            if (!name_override.empty()) nm = name_override;
    ...
    app.add_subcommand("list", "list installed agents")
        ->callback([] {
            auto paths = Paths::discover();
            if (!paths) { std::println(stderr, "error: {}", paths.error().message); return; }
            Registry reg{*paths};
            auto agents = reg.list();
            if (!agents) { std::println(stderr, "error: {}", agents.error().message); return; }
            if (agents->empty()) { std::println("no agents installed"); return; }
